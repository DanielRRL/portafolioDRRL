...
import styles from './ClosingPlasma.module.css'
...
export default function ClosingPlasma({
  speed = 1,
  turbulence = 1,
  mouseInfluence = 1,
  grain = 1,
  ...
  const isDarkRef = useRef(theme === 'dark' ? 1 : 0)

  useEffect(() => {
    isDarkRef.current = theme === 'dark' ? 1 : 0
  }, [theme])
  ...
  useEffect(() => {
    ...
    if (
      !uRes || !uTime || !uMouse || !uIsDark ||
      !uSpeed || !uTurbulence || !uMouseInfluence ||
      !uGrain || !uSparkle || !uVignette || !uOpacity ||
      !uDarkA || !uDarkB || !uDarkC ||
      !uLightA || !uLightB || !uLightC
    ) {
    ...
    const darkA = hexToRgb01(settings.darkColorA)
    const darkB = hexToRgb01(settings.darkColorB)
    const darkC = hexToRgb01(settings.darkColorC)
    const lightA = hexToRgb01(settings.lightColorA)
    const lightB = hexToRgb01(settings.lightColorB)
    const lightC = hexToRgb01(settings.lightColorC)
